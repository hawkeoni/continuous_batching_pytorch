...
class ContinuousBatcher(Batcher):
    ...
    def _update_prefill(self, batch: _Batch):
        ...
        if not batch.texts_decoding:
            with self._record_function(f"Prefill attention & positions"):
    ...
    def _expand_batch(
        ...
        with self._record_function(f"Expand attention"):
        ...
        with self._record_function(f"Expand kv cache"):
    ...
    def _step(self, batch: _Batch):
        with self._record_function(f"Step"):
    ...
    def _get_finished_samples(self, batch: _Batch):
        with self._record_function(f"Finished check"):
            finished = (
                (
        ...
        with self._record_function(f"Finished index_select"):
